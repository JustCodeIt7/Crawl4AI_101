...
async def main() -> None:
    ...
    pruning = DefaultMarkdownGenerator(content_filter=PruningContentFilter(threshold=0.45, threshold_type="dynamic", min_word_threshold=5))

    # Configure a BM25-based generator — the query string steers which blocks
    # are considered relevant to the topic we care about
    bm25 = DefaultMarkdownGenerator(content_filter=BM25ContentFilter(user_query="installation quickstart browser config", bm25_threshold=1.0))
